...
public class NettyHttpClientHandler extends SimpleChannelInboundHandler<FullHttpResponse> {
    ...
    public NettyHttpClientHandler(final NettyHttpClient nettyHttpClient) {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpResponse msg) throws Exception {
        ...
        Serializer serializer = nettyHttpClient.getSerializer();
        XxlRpcResponse xxlRpcResponse = (XxlRpcResponse) serializer.deserialize(responseBytes, XxlRpcResponse.class);

        // notify response
//        nettyHttpClient.notifyInvokerFuture(xxlRpcResponse.getRequestId(), xxlRpcResponse);
        BiConsumer notifyInvokerCallback = nettyHttpClient.getNotifyInvokerCallback();
        notifyInvokerCallback.accept(xxlRpcResponse.getRequestId(), xxlRpcResponse);
    }
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof IdleStateEvent){
            ...
            nettyHttpClient.writeAndFlush(ctx.channel(), Beat.BEAT_PING);    // beat N, close if fail(may throw error)
