package com.xxl.rpc.remoting.net.netty.server;
...
public class NettyHttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public NettyHttpServerHandler(final ThreadPoolExecutor serverHandlerPool, Serializer serializerInstance,
                                  Server server) {
...
//        Serializer serializerInstance = xxlRpcProviderFactory.getSerializerInstance();
        try {
            if ("/services".equals(uri)) {    // services mapping
                ...
                Map<String, Object> serviceData = server.getServiceData();
                for (String serviceKey : serviceData.keySet()) {
                    stringBuffer.append("<li>").append(serviceKey).append(": ").append(serviceData.get(serviceKey)).append("</li>");
                }
                ...
                // request deserialize
                XxlRpcRequest xxlRpcRequest = (XxlRpcRequest) serializerInstance.deserialize(requestBytes, XxlRpcRequest.class);
                ...
                // invoke + response
                XxlRpcResponse xxlRpcResponse = server.invokeService(xxlRpcRequest);

                // response serialize
                byte[] responseBytes = serializerInstance.serialize(xxlRpcResponse);
